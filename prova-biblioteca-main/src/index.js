...
async function iniciarBanco() {
  ...
  const total = await database.get('SELECT COUNT(*) AS total FROM livros');
...
app.put('/livros/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const livroExistente = await db.get(
      'SELECT * FROM livros WHERE id = ?',
      [id]
    );
    ...
    const novaDisponibilidade = livroExistente.disponivel === 1 ? 0 : 1;
    ...
    const livroAtualizado = await db.get(
      'SELECT * FROM livros WHERE id = ?',
      [id]
    );
